...
cert_valid_from="2024-01-01 14:00:00"
...
#   - PAI Subject includes PID
{
    ...
    "$chip_cert_tool" gen-att-cert --type a --subject-cn "Infineon Matter Test PAA" --subject-vid "$vid" --valid-from "$cert_valid_from" --lifetime "$cert_lifetime" --out-key "$paa_key_file".pem --out "$paa_cert_file".pem

    pai_key_file="$dest_dir/Chip-Test-PAI-$vid-$pid-Key"
    pai_cert_file="$dest_dir/Chip-Test-PAI-$vid-$pid-Cert"

    "$chip_cert_tool" gen-att-cert --type i --subject-cn "Infineon Matter Test PAI" --subject-vid "$vid" --subject-pid "$pid" --valid-from "$cert_valid_from" --lifetime "$cert_lifetime" --ca-key "$paa_key_file".pem --ca-cert "$paa_cert_file".pem --out-key "$pai_key_file".pem --out "$pai_cert_file".pem
    ...
    for dac in "${dac_ids[@]}"; do
        ...
        "$chip_cert_tool" gen-att-cert --type d --subject-cn "Infineon Matter Test DAC $dac" --subject-vid "$vid" --subject-pid "$pid" --valid-from "$cert_valid_from" --lifetime "$cert_lifetime" --ca-key "$pai_key_file".pem --ca-cert "$pai_cert_file".pem --out-key "$dac_key_file".pem --out "$dac_cert_file".pem
    done
}
