    ...
    do 
    {
        ...
        else
        {
            ...
            if (OPTIGA_LIB_SUCCESS != return_status)
            {
                ...
                optiga_lib_print_message("optiga_crypt_hmac_start api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
                break;
            }
            ...
            while (remainingLen > 0)
            {
                ...
                if (remainingLen > MAX_MAC_DATA_LEN)
                {
                    ...
                    if (OPTIGA_LIB_SUCCESS != return_status)
                    {
                        ...
                        optiga_lib_print_message("optiga_crypt_hmac_update api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
                        break;
                    }
                }
                else
                {
                    ...
                    if (OPTIGA_LIB_SUCCESS != return_status)
                    {
                        ...
                        optiga_lib_print_message("optiga_crypt_hmac_finalize api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
                        break;
                    }
                }
            }
...
{
    optiga_lib_status_t return_status;
    uint8_t header256[] = {0x30,0x59,0x30,0x13,0x06,0x07,0x2A,0x86,0x48,0xCE,0x3D,
    ...
    do 
    {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status = optiga_crypt_ecc_generate_keypair(p_local_crypt,
                                                          curve_id,
                                                          key_type,
                                                          FALSE,
                                                          &optiga_key_id,
        ...
        if (OPTIGA_LIB_SUCCESS != return_status)
        {
            ...
            optiga_lib_print_message("optiga_crypt_ecc_generate_keypair api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
            break;
        }
...
void trustmGetKey(uint16_t optiga_oid,uint8_t *pubkey,uint16_t *pubkeyLen)
{
    ...
    do 
    {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status = optiga_util_read_data(p_local_util,
                                              optiga_oid,
                                              offset,
                                              pubkey,
                                              pubkeyLen);                                                  
...
optiga_lib_status_t trustm_ecdsa_sign(optiga_key_id_t optiga_key_id, uint8_t *digest, uint8_t digest_length, uint8_t *signature, uint16_t *signature_length)
{
    ...
    do 
    {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status = optiga_crypt_ecdsa_sign(p_local_crypt,
                                                digest,
                                                digest_length,
                                                optiga_key_id,
                                                signature,
        ...
        {
            //optiga_crypt_ecdsa_sign api returns error !!!
            optiga_lib_print_message("optiga_crypt_ecdsa_sign api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
            break;
        }
        while (optiga_lib_status == OPTIGA_LIB_BUSY);
...
optiga_lib_status_t trustm_ecdsa_verify(uint8_t *digest, uint8_t digest_length, uint8_t *signature, uint16_t signature_length,uint8_t *ecc_pubkey,uint8_t ecc_pubkey_length)
{
    ...
    do 
    {
        ...
        return_status = OPTIGA_LIB_BUSY;
        return_status = optiga_crypt_ecdsa_verify(p_local_crypt,
                                                digest,
                                                digest_length,
                                                signature,
                                                signature_length,
        ...
        if (OPTIGA_LIB_SUCCESS != return_status)
        {
            ...
            optiga_lib_print_message("optiga_crypt_ecdsa_verify api returns error !!!",OPTIGA_UTIL_SERVICE,OPTIGA_UTIL_SERVICE_COLOR);
            break;
        }
        while (optiga_lib_status == OPTIGA_LIB_BUSY);
    } while (0);
